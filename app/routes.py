...
def rules():
    ...
    next_game = get_next_game()
    return render_template("rules.html", title='Rules', winner_bet=winner_bet, game_id=next_game.id)
...
def standings():
    ...
    next_game = get_next_game()
    return render_template("standings.html", title='Standings', results=results, game_id=next_game.id)
...
def games(idd):
    games = Game.query.order_by(Game.id.asc()).all()
    #bets = Bet.query.filter(Bet.user_id==current_user.id).all()
...
def user(username):
    ...
    next_game = get_next_game()
    return render_template('user.html', title='Profile', user=user, bets=bets, stats=stats, winner_bet_points=winner_bet_points, game_id=next_game.id)#, posts=posts.items, next_url=next_url, prev_url=prev_url)
...
def edit_profile():
    ...
    next_game = get_next_game()
    return render_template('edit_profile.html', title='Edit Profile', form=form, game_id=next_game.id)                         
...
def upload_game_results():
    ...
    if form.validate_on_submit():
    	...
    	for bet in bets_to_update:
    		...
    		bet.points = int(bet.first_goal_correct) + 3*int(bet.winner_correct) + int(bet.score_diff_correct) + 3*int(bet.score_correct) ## + 2*int(bet.score_correct) for football
